...
// 获取数据库路径（用户目录下的 .warehouse-app/warehouse.db）
...
export function claimCreate(row: {
  ...
  if (row.items?.length) {
    const ins = d.prepare('INSERT INTO claim_items (claim_id, item_id, requested_qty) VALUES (?, ?, ?)');
    for (const it of row.items) {
      ins.run(claimId, it.item_id, it.requested_qty);
    }
  }
  return { id: claimId };
}
...
export function importBackup(buffer: ArrayBuffer) {
  const dbPath = getDbPath();
